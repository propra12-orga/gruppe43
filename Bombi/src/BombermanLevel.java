...
public class BombermanLevel {
    ...
    // ein paar Konstanten, um das Manipulieren des Felder einfacher zu
    // gestalten
     ...
     * 
     * @param width
     *            : Breite des Spielfelds in Pixel
     * @param height
     *            : Hoehe des Spielfelds in Pixel
     **/
        ...
        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                ...
                // gehe sicher, dass alle vier Spieler anfangs eingemauert sind
                        ...
                        && ((j >= 0 && j <= 2) || (j >= height - 3 && j <= height - 1)))
                    tiles[i][j] = STONE;
                else if ((j == 2 || j == height - 3)
                        && ((i >= 0 && i < 2) || (i > width - 3 && i <= width - 1)))
                    tiles[i][j] = STONE;
